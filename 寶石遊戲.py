...
class Fall(pygame.sprite.Sprite):
    ...
    def reset(self):
        self.type = random.choices(["gem1", "gem2", "gem3", "bomb"],weights=[45, 28, 15, 12],k=1)[0]
...
while running:
    ...
    time_left = max(0, 60 - int(past_seconds))
    degree = min(past_seconds / 60,1)
    player.update()
    for item in items:
        item.update(degree)
    ...
    if past_seconds >= 60:
        ...
        result = draw_game_over(score,high_score)
